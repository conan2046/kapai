...
CREATE TABLE IF NOT EXISTS `answer_settings` (
  ...
  `fallback_reward_type` INTEGER NOT NULL DEFAULT '60000',
  `fallback_reward_amount` INTEGER NOT NULL DEFAULT '1000'
);

CREATE TABLE IF NOT EXISTS `answer_daily_progress` (
  `role_id` INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS `happy_wheel_log` (
  `id` INTEGER PRIMARY KEY AUTOINCREMENT,
  `role_id` INTEGER NOT NULL DEFAULT '0',
  `data` TEXT NOT NULL DEFAULT '',
  `created_at` INTEGER NOT NULL DEFAULT '0'
);
CREATE INDEX IF NOT EXISTS `idx_happy_wheel_log_role_time`
  ON `happy_wheel_log` (`role_id`, `created_at`);
...
INSERT INTO sqlite_sequence(name,seq) VALUES('question',38);
