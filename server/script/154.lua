...
MonsterName = LANGUAGE_TRANSFORM_1878
LEVEL_LIMIT = 30
...
function NpcMain(pUser,missionId,index)
	...
	Option(pUser,MonsterName,LANGUAGE_TRANSFORM_1879,LANGUAGE_TRANSFORM_1880)
...
function MonsterCallBack(pUser,sel)
	if sel == 1 then
		if pUser:GetLevel() < LEVEL_LIMIT then
			Dialog(pUser,MonsterName,LANGUAGE_TRANSFORM_1881..LEVEL_LIMIT..LANGUAGE_TRANSFORM_1882)
			return
		end
