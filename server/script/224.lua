...
local bless = {LANGUAGE_LLD_0034,LANGUAGE_LLD_0035,LANGUAGE_LLD_0036,LANGUAGE_LLD_0037,LANGUAGE_LLD_0038,LANGUAGE_LLD_0039,LANGUAGE_LLD_0040,LANGUAGE_LLD_0041,
				LANGUAGE_LLD_0042,LANGUAGE_LLD_0043}
...
function NpcMain(pUser,missionId)
	...
	opt = opt .. "1|" ..LANGUAGE_LLD_0032.. "|"
	
	s = string.format(LANGUAGE_LLD_0033, costYB)
	opt = opt .. "2|"..s.."|"

	opt = string.sub(opt,1,-2)
	if #opt ~= 0 then
		Option(pUser,NPCName,LANGUAGE_LLD_0031,opt)
		pUser:SetCallFun("NpcMainSel")
	else
		Dialog(pUser,NPCName,LANGUAGE_LLD_0031)
	end
end
...
function NpcMainSel(pUser,sel)
	if sel == 1 then
		...
		if pUser:GetExtData8(387) >= 5  then
			Dialog(pUser,NPCName,LANGUAGE_CHY_296)
			return
		end
		if pUser:GetExtData32(291) +5*60 > j.GetSystemTime() then
			Dialog(pUser,NPCName,LANGUAGE_CHY_297)
			return
		end
