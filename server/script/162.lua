...
function NpcMain(pUser,missionId)
	...
	if s ~= nil then
		...
		if tonumber(t[1]) == 2 then
			if tonumber(t[6]) == 2 then
				j.ShowFinishYaYunBiaoChePanel(pUser)
--				Option(pUser,"邱镖头","老子信了你的邪！","3|完成押运镖车")
			else
				j.ShowYaYunBiaoChePanel(pUser,0,163)
--				Option(pUser,"邱镖头","老子信了你的邪！","1|我要换车|2|不换车")
			end
...
--			j.ShowYaYunBiaoChePanel(pUser)
		...
		Dialog(pUser,NPCName,LANGUAGE_TRANSFORM_2111)
	end
end
...
--		end
		...
		if q == 4 or (q == 3 and math.random(1,100) <= 20) then
			j.SysInfoToAllUser("[c"..ROLE_NAME_COLOR.."]"..pUser:GetName()..LANGUAGE_TRANSFORM_2112..ITEM_NAME_COLOR.."]"..j.GetSceneName(pUser:GetSceneId())..LANGUAGE_TRANSFORM_2113..GongGaoColor[q][1].."]"..GongGaoColor[q][2]..LANGUAGE_TRANSFORM_2114)
		end
		...
		if q >= 1 then
			j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_2115..GongGaoColor[q][2]..LANGUAGE_TRANSFORM_2116)
		else
			j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_2117)
		end
		...
		if worldExpPer > 0 then
			...
			j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_2122..exp..LANGUAGE_TRANSFORM_2123..money..LANGUAGE_CHY_301..j.GetItemName(item_id).."*"..item_num.."[/c]")
		end
...
--		j.ShowYaYunBiaoChePanel(pUser,0)
	elseif sel == 3 then
		...
		j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_2125)
		pUser:RecoveryAllHp()
		pUser:AddExpByItemWithTips(exp)
--		pUser:AddBaiHuaChip(3,1,1)
		
		pUser:AddMoney(money)
		j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_2126..money.."[/c]")
