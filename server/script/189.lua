...
function NpcMain(pUser,missionId,index)
	...
	if lv >= XCS_LEVEL_LIMIT then
		Option(pUser,name,LANGUAGE_TRANSFORM_4679..name..LANGUAGE_TRANSFORM_4681,LANGUAGE_TRANSFORM_4682)
	else
		Option(pUser,name,LANGUAGE_TRANSFORM_4683..name..LANGUAGE_TRANSFORM_4685,LANGUAGE_TRANSFORM_4686)
	end
...
function MonsterCallBack(pUser,sel)
	...
	if sel == 1 then
		if pUser:GetLevel() < LEVEL_LIMIT then
			Dialog(pUser,name,LANGUAGE_TRANSFORM_4687..LEVEL_LIMIT..LANGUAGE_TRANSFORM_4688)
			return
		end
		
		if pUser:GetTeam() == 0 then
			Dialog(pUser,name,LANGUAGE_TRANSFORM_4689)
			return
		end
		if pUser:GetTeam() ~= pUser:GetRoleId() then
			Dialog(pUser,name,LANGUAGE_TRANSFORM_4690)
			return
		end
		if j.GetTeamMemNum(pUser) < 2 then
			Dialog(pUser,name,LANGUAGE_TRANSFORM_4691)
			return
		end
		...
		if pUser:IsXunChaShiKilled(NPCID,index) then
			Dialog(pUser,name,LANGUAGE_TRANSFORM_4696)
			return
		end
