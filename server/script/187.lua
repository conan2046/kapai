...
function NpcMain(pUser,missionId,index)
	...
	if lv >= XCS_LEVEL_LIMIT then
		Option(pUser,name,LANGUAGE_TRANSFORM_163..name..LANGUAGE_TRANSFORM_165,LANGUAGE_TRANSFORM_166)
	else
		Option(pUser,name,LANGUAGE_TRANSFORM_167..name..LANGUAGE_TRANSFORM_169,LANGUAGE_TRANSFORM_170)
	end
...
function MonsterCallBack(pUser,sel)
	...
	if sel == 1 then
		if pUser:GetLevel() < LEVEL_LIMIT then
			Dialog(pUser,name,LANGUAGE_TRANSFORM_171..LEVEL_LIMIT..LANGUAGE_TRANSFORM_172)
			return
		end
		
		if pUser:GetTeam() == 0 then
			Dialog(pUser,name,LANGUAGE_TRANSFORM_173)
			return
		end
		if pUser:GetTeam() ~= pUser:GetRoleId() then
			Dialog(pUser,name,LANGUAGE_TRANSFORM_174)
			return
		end
		if j.GetTeamMemNum(pUser) < 2 then
			Dialog(pUser,name,LANGUAGE_TRANSFORM_175)
			return
		end
		...
		if pUser:IsXunChaShiKilled(NPCID,index) then
			Dialog(pUser,name,LANGUAGE_TRANSFORM_180)
			return
		end
