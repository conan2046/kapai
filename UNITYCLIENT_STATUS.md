...
| 帮派 | 下一批 | 未开始 | `/54` 单独取证和验证，不与 Team 混跑 |
...
- 最新收口模块：Team 队伍系统第一阶段。
- 隔离账号：`userId=7200059`；目标角色 `1000117`；同伴角色 `1000118`。
- 状态闭环：空态 → 创建 → 邀请/同伴接受 → 双人权威状态/UI → 离队 → 持久化空态；`/30` 推送重拉有效。
- Unity 结果：BatchMode 编译/运行通过；严重异常 `0`。
- GameView：双人队与最终空态均为 `1334×750`，五槽、玩家摘要和按钮完整可见。
- UI 转换测试：`10/10`。
...
下一批：`Guild /54`，与 Team 分离取证、实现和变更型验证。

固定顺序：

1. 锁定 `/54` 帮派操作、全量/增量字段、旧客户端入口和真实 Prefab。
2. 复用 `PlayerSummary` 与现有通用列表/奖励/红点能力。
3. 先做只读帮派状态，再选择低风险、可回收的变更闭环。
4. 不复用 Team 最终验证账号，不把 Team 深化项混入 Guild。
