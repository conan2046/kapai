...
- 最新收口模块：World / 战斗 / 副本系统第一阶段；`main` 基线 `d46723ab8e2bb5465f1e22585f148eb57f02443f`（含 Guild `b6537c2`）。
- 最终隔离账号：`userId=7200008`；过程账号 `7200004-7200007` 不作为最终证据，Team/Guild 账号未复用。
- 状态闭环：`/320 op=1/2/27` 世界/章节/关卡/详情 → `op=5` 挑战 → `op=8` 六项结算 → 重拉确认关卡 10001 三星持久化。
- Unity 结果：BatchMode 编译/运行通过；严重异常 `0`。
- GameView：地图、详情、结算、最终三星态四张 `1334×750`；奖励图标、名称和数量完整可见。
- UI 转换测试：`10/10`。
...
本批：`World /320` 第一阶段已完成，与 PvP 和战斗表现深化分离取证、实现和变更型验证。

固定顺序：

1. `/320 op=1/2/5/8/27`、服务端、旧客户端入口和真实 Prefab 已形成模块证据。
2. WorldStore/Presenter/Lua 复用 Hero、Formation、Reward、ResourceService、VirtualList 和 UiStack。
3. 最终变更闭环为一次性账号挑战关卡 10001，结算后重拉确认三星；未领取宝箱、未扫荡/重置。
4. `/21-23` 战斗表现缺服务端分发证据，未做旁路；PvP、技能特效、自动战斗未混入。
5. 后续模块继续按统一工具链单独立项。
