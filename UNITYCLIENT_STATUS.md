...
| Validated | `9/17 = 52.9%` | Steam已排除12个模块；PlayerHud、HeroEquip、GameplayShops、Gameplay、Draw 已因当前门禁/证据/协议缺口撤销完成态，剩余17个模块中9个仍保持 G0-G6 有效 |
...
| 装备/法宝 | `G0-G3 passed / G4-G6 pending` | 已重新完成当前源码 G0-G3；修复 `/319 → /70 PRO_UPDATE_PET_INFO` 神将属性/气血/战力刷新、法宝卸下重算顺序和固定账号可逆夹具。原始批处理业务链与 6/6 语义断言已成功，但正式 Runner 被当前 `33 expected / 0 actual` 真实控件覆盖门禁拦截 | 从 `hero-equip-g6` 接入 33 个真实 Button/EventSystem 操作，生成六个 Unity 状态及 Cocos/Unity 差异报告；随后用标准批处理摘要依次关闭 G4、G5、G6。历史 20/20、33/33 和 MCP 结果不得复用 |
...
| 抽卡 | `G0-G5 retained / G6 audit missing` | 历史覆盖 `btn_zhaomu → HappyDrawUI → /224`、28控件及 Draw→Hero→Formation 闭环；当前 `DRAW_CONTROLS.json` 缺少硬门禁 v2 的 `g6Audit`，完成态已撤销 | 补齐真实 G6 审计并由中央文档/矩阵测试通过后才能恢复完成态；概率公示、支付渠道不在本模块 |
| 玩法大厅 | `G0 passed / G1-G6 pending` | 原13项/16控件历史门禁已被 Steam 过滤输入变更失效；当前运行列表为游历三界、封神列传、竞技场、法宝搜索、每日任务，共5项/8控件 | 按当前5项/8控件重新采集 Cocos 基线并完成 G1-G6；旧截图、Runner与哈希仅作诊断 |
...
- 背包 G0-G6（2026-08-21）：从当前真实入口 `Layer/Main_UI/ButtonGroup1/btn_Bag` 重新完成 hard-gate v2。固定账号 `7200057/1000115` 的 26/26 真实控件、14/14 语义断言和 10/10 双端 `1334×750` 状态对照通过；真实 `/8`、`/15` 覆盖全量、增量、整理、直接/批量/任选使用、错误分支、关闭重进、断线重连、持久化、切号及精确恢复。Fixture 与 Computer Use 残留均为0，严重异常0；两次正式 BuildBatch SHA-256 均为 `090ABA5905E35B4E965C16F044DD19F0D7A3F2E4E7C9DE131E6E2ABC8B426CA4`，自动复盘52/52已解决、未解决0。证据 `.local/unity-validation/bag-fixed-account-latest.json`、`.local/ui-fidelity/Bag/compare/g5-20260821/report.json`、`.local/unity-validation/bag-retrospective-latest.json`。
...
| 保留 | 法宝搜索、游历三界、封神列传、竞技场 | 前三项是单人权威状态；竞技场存在40份AI配置、1万名次机器人初始化和真实机器人战斗链 |
## 6. 已知风险
