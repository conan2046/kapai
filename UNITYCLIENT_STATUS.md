...
| Validated | `1 个模块迁移完整` | 神将/阵容已按新标准完成 G0-G6、16/16真实控件和双端视觉验收；其他历史完成结论仍待重审 |
...
- 神将/阵容 G5双端证据与人工验收（2026-07-26）：用户放宽禁止前台限制后，固定账号 `7200057 / roleId=1000115`、同数据、原生 `1334×750` 已重采 Cocos 16/16、Unity 16/16 原图，并生成16组并排、叠加、差异和聚合报告。Unity阵容入口补齐 `/319 op=1/17` 预取后，强化大师真实打开且16项Button自动化通过。人工视觉仅 `2/16`：空阵位、布阵弹窗通过，其余14项因公共层、灰色占位、养成缺内容、强化大师错误页面、装备/法宝详情错误复用和属性弹窗残缺失败。证据 `.local/ui-fidelity/Hero/compare/g5-live-20260726/manual-acceptance.md`；G5-G6保持pending。
...
## 4. 当前批次
《道友来封神》神将/阵容已完成 G0-G6、16/16控件和视觉收口。当前任务到此结束；按“一功能一个任务”规则更新HANDOFF并新开任务后，才能选择下一个模块。
