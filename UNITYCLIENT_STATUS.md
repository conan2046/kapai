...
| 神将/阵容 | `logic-validated-visual-pending` | `/24、/48` 列表、详情、阵位变更/恢复；旧 `LCPet/LCFormation` 语义回到 Lua，C# 暂作渲染镜像 | 先补有效 Cocos 同状态截图并修正占位文字、错误图文、遮挡和布局差异，再做通用 Lua UI Bridge |
...
本批停止“可运行即推进”口径，严格执行 `docs/unityclient/MIGRATION_SOP.md` 的 G0-G6 门禁。同一时间只处理阵容；未取得有效 Cocos 基准、未清零视觉差异或未完成回归收口时，不得开启下一模块。

当前顺序：补齐阵容 Cocos 各状态截图 → 完成 Lua/节点/资源映射审查 → 修复占位文图、公共层遮挡、布局、按钮红点和 Imod → 重验 `/24、/48`、重拉、重连与切号 → 生成逐状态叠加/差异报告 → 全门禁通过后升级 `visual-1to1-complete`。

新模块默认禁止新增业务型 `Store/Catalog/Presenter`；旧 Lua 作为业务与权威状态，C# 只做通用 Bridge 和渲染适配。阵容完整通过后，才评估向其他模块推广。
