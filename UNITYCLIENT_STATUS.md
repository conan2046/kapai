...
| 神将/阵容 | `partial-interactive-audit-required` | 仅确认 `/24、/48` 数据显示、阵位/背包卡选择和自动化换位；养成、强化大师、替换、空位上阵、装备槽、详细属性、真实布阵按钮等未绑定 | 旧 G0-G6 全部撤销；从完整控件矩阵重新执行 G0 |
| 装备/法宝 | `partial-interactive-audit-required` | `/319` 数据、穿脱/强化协议诊断通过，但尚未证明所有可见入口、详情、更换、强化按钮均由玩家真实点击闭环 | 保留 G0-G3 取证/实现参考；G4-G6 重做逐控件人工与自动验收 |
...
## 3. 最新验证基线
- 《道友来封神》Cocos 当前版本普查（2026-07-19）：全仓 605 Lua/1909 控件候选结果作废；以当前 `MainUI` 为根构建产品闭包后，126 条 AppDef 配置路由中 42 条静态追达、84 条未归属，闭包为 57 Lua/312 控件候选、主界面 42 个事件绑定。机器清单位于 `tools/cocos-audit/generated/`。阵容重新登记 16 个控件/状态类，Unity 仅发现公共关闭、已占用阵位选择、布阵入口 3 个 Listener，但均缺全套门禁，因此完成 `0/16`；矩阵为 `docs/unityclient/matrices/HERO_CONTROLS.json`。本轮 Cocos 主界面已确认；Win32 系统/窗口 DPI 已是 96/100%，但模拟器客户区仍为 1067×600，未达到配置的 1334×750，阵容真实入口本轮未形成有效证据，白屏图全部作废。
...
## 4. 当前批次
新模块迁移暂停。当前先完成《道友来封神》全局入口普查与逐控件冻结：已生成首版产品静态闭包和阵容 16 项矩阵；下一门禁是保持 100% DPI、先修正模拟器客户区为 1334×750，再补当前运行可达性并冻结阵容 G0，证据未齐前不编码。
...
基础设施完成后只恢复装备/法宝 G5 补证，不开启新模块；商城、邮件第二阶段及装备深度培养继续暂停。
