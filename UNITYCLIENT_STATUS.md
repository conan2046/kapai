...
| 项 | 当前值 |
|---|---|
| 唯一活动范围 | `Unity 现有功能真实 Play / 协议 / SQLite Bug 验收` |
| 当前门禁 | `Unity 客户端迁移完成；当前只按现有功能做定向 Bug 回归，不再重开迁移 G0-G6` |
| 当前阻塞 | `无；World 三类战斗公共状态隔离已完成并推送主干` |
| 下一步 | `按用户反馈选择 Unity 现有功能 Bug，执行最小复现、修复、真实 Play 回归和定向提交` |
