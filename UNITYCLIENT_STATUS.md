...
| Validated | `14/17 = 82.4%` | Steam已排除12个模块；剩余17个模块中14个已按新标准完成 G0-G6 |
...
| 玩法大厅 | `G0-G6 passed；Steam过滤已静态接入` | 原13项/16控件历史门禁已通过；Steam过滤后运行列表为游历三界、封神列传、竞技场、法宝搜索、每日任务，共5项/8控件 | 完成一次Gameplay回归后刷新Steam版截图与控件证据 |
...
当前冻结：已排除12个模块，严格完成率 `14/17 = 82.4%`。
