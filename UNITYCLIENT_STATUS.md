...
| 装备/法宝 | `g2-complete`（G3 进行中） | G0-G2 全部通过；隔离角色装备 5、法宝 3、碎片 6 组；18 张原生 Cocos 图覆盖冻结状态；G3 已建立 Lua 装备/法宝权威模型、统一 `/319` op 路由、断线清理与单次强化事件，Runner 穿脱/强化通过 | 继续 G3：拆分装备/法宝页面并挂接更换、强化真实 Prefab；C# 仅保留 Bridge/渲染 |
...
装备/法宝 G0-G2 已全部通过，18 张 Cocos 原生基准覆盖冻结状态，动态协议与最终重连状态通过。状态仍以最后完成门禁记为 `g2-complete`，G3 已开始：Lua 权威镜像、共享 `/319` 单次读 op 路由、断线清理及单次强化事件已接入并通过 Runner；下一步拆分装备/法宝页面并挂接更换、强化真实 Prefab。仍不得继承阵容的视觉完成结论。
