...
## Agent 搜索与输出限流
- 禁止把客户端、服务端和资源目录的宽泛 `rg` 结果并行完整回传到对话窗口；大量终端文本会造成 Codex 桌面端渲染卡顿。
- 搜索时先用 `rg -l`、精确目录和文件类型缩小文件集合，再读取少量上下文；单次展示结果原则上不超过 200 行。
- 必须保留完整搜索结果时，重定向到 `.local/` 临时文件，只向对话输出命中数量、文件列表和必要片段。
- 日志默认只读取 `-Tail 100` 或更少；需要扩大范围时分段读取，不一次回传整份日志。
- 不并行执行多个可能产生大量文本的搜索；先检查上一条结果规模，再决定是否继续扩大范围。
