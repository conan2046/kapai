...
### 5.2 主界面体力加号按钮（OpenUseUI）

- `MainUI.lua:393-398` 的 `Icon_tili/AddBtn` 点击回调调用 `Utils:OpenUseUI(500,1)`，打开 `Common.ItemUseUI`（道具使用界面），针对道具 `id=500` 体力丹（`item_dat.lua:440`，`des="使用增加25点体力"`，`sub_value={{60026,25}}`）。
- 这是“使用体力丹”界面，**不是购买 / 商城界面**；代码注释 `--购买体力丹` 与实际调用 `OpenUseUI`（使用）不一致。真正购买须走 商城（`item_from="来源：商城"`）。
- 若玩家未持有体力丹，该界面无可使用道具；此入口与已排除的 `StaminaClaim` 领取界面无关。
