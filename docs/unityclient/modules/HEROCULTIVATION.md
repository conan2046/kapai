...
- G1 已按扩展后的源码/资源输入指纹重新通过：固定身份 `userId=1 / roleId=1000001`，15 个原状态逐项复核并新增修炼说明第二页、说明属性详情、数量输入，共冻结 18 个当前 Cocos 原生 `1334×750` 状态。
- G2 已通过：培养写入归 `/24 op1/3/4/5/6/7/12`，`/25 op2` 为完整神将权威推送，`/48` 只读阵位快照供左右切换，`/70` 刷新属性/战力；写操作必须串行并在回包后强制重查 `/24` 与 `/48`。
- 服务端已修复修炼零次数/超剩余扣除和天命未满先扣激活材料问题，并补齐 `xiulian_cost=852`；Windows 服务端当前源码编译通过。
...
- 修炼：属性进度、帮助两页签、1-20级选择、说明属性详情、材料、单次/一键/指定次数修炼、天命激活及对应成功/不足。
