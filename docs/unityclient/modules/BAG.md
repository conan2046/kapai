...
- Steam SQLite S5：同一服务端二进制、隔离新角色和8个确定性步骤在SQLite/MySQL各返回52个响应；三次`/8`与六次`/15`原始 payload 均字节一致。逐操作确认`3201×1`新增、整理成功、直接使用后权威删除、`1111×2`任选礼包使用后新增`4621×1`且源物品减为1；两端正常退出并重启后`/8`仍字节一致，数据库`package/save_data/mission`三项SHA完全一致。证据`.local/unity-validation/steam-sqlite-s5-bag-latest.json`。
