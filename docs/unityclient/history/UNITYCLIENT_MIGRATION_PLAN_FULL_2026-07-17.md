...
当前完成：`/221 op=1/op=2`、`ShopStore/ShopCatalog/ShopPresenter/ShopController`、真实 `shop/shangcheng.prefab`、17 件基础商品列表、购买次数/限购、每日刷新倒计时、货币显示、MessageBox 购买确认、RewardPresenter、隔离角色严格单次购买及购买后权威列表复查、`1334×750` 视觉 QA。待完成：`op=3` 手动刷新、特殊商店、完整空态/错误/断线组合回归。
...
本批已完成：`Shop Store` → 商品列表 → 限购/刷新时间 → 货币显示 → 购买确认 → 隔离角色单次购买。

下一批建议：`Friend Store` → 好友列表/申请列表 → 添加/同意/拒绝/删除的状态闭环。商城手动刷新与特殊商店留在 C5 第二阶段；邮件红点、一键领取和删除留在 C4 第二阶段；装备深度培养留在 C3 后续批次。
