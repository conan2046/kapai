...
## 2026-07-17 12:30 Shop Store 第一阶段收口

- 已完成 `/221 MSG_SHOP` 三方取证和 Unity 迁移：`ShopStore + ShopCatalog + ShopPresenter + ShopController.lua`，覆盖基础商店商品列表、购买次数/限购、服务器时间驱动的每日刷新倒计时、货币展示、购买确认、单次购买及购买后权威列表复查。
- 主界面真实入口 `Layer/Main_UI/ButtonGroup5/btn_shangcheng` 与弹窗入口 `Layer/Main_UI/tankuang1/btn_shangcheng` 已接入；页面实例化真实 `shop/shangcheng.prefab`，商品区复用 `VirtualList` 三列布局、`ResourceService`、`ServerTimeService`、`CurrencyStore`、`RewardStore/RewardPresenter`、MessageBox 和返回栈。
- 已将服务端权威 `shop.json/item.json` 同步到 Unity Configs。基础商店共渲染 17 件商品，图标缺失 `0`；按服务端实际扣款路径，配置货币 `60001` 在本地服由 `AddTongBao(type=0)` 扣除并展示为非绑定元宝 `60003`，未改服务端逻辑。
- 全新隔离角色 `userId=7172217` 实测商品 `1001` 严格购买一次：限购计数 `0→1`、元宝 `100000→99980`、获得金币 `100000`，再次 `/221 op=1` 确认购买次数持久化为 `1`。
- Unity batchMode 实际完成 C# 编译和真实本地服 PlayMode，`Tundra build success`、无 C# warning、严重异常 `0`；结果为 `COMPLETE: /221 list -> ShopStore/limits/server time/currency -> confirmed single purchase id=1001 -> persisted count=1`。
- GameView 验收固定为 `1334×750`；`bootstrap-shop-confirm.png` 与 `bootstrap-shop.png` 已确认购买文案、限购、倒计时、花费/拥有及购买后数值。UI 转换测试 `10/10`。
- 本阶段未改写 `shop/shangcheng.prefab`，未修改服务端，未 stage、commit、push。按完整可玩客户端业务加权口径约完成 `20%`，静态 Prefab 仍为 `356/356`。

下一迁移建议：进入 `Friend Store`，先做好友列表、申请列表、添加/同意/拒绝/删除的只读与低风险状态闭环；商城手动刷新、特殊商店和错误/空态组合回归留在 C5 第二阶段。
