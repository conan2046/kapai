...
| `Team` | 队伍 | 入口隐藏 |
| `Guild` | 帮派、宗门 | 入口隐藏 |
| `Activity` | 运营活动 | 整体排除；当前集合含充值及全服排行榜/其他玩家数据 |
...
- 竞技场：仅因服务端存在40份竞技场AI配置、1万机器人名次初始化和真实机器人战斗链而保留；不得改为依赖真人在线数据后仍沿用当前结论。
- 玩法大厅 Steam 列表固定为 `function_id=1/3/6/9/10`。其他 `function_id` 即使仍在原始 Cocos 配置中，也不得由 Unity Steam 列表展示。
