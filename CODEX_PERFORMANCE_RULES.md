...
- 新增并行任务前评估本机负载；系统 CPU 持续超过 70%、16 GB 内存机器可用内存低于 4 GB（低于 2 GB 时停止重任务）或磁盘持续高负载时，停止新增任务并提醒用户。
...
| 可用内存（本机 16 GB） | 低于 4 GB 预警；低于 2 GB | 停止新增任务；低于 2 GB 停止重任务 |
...
- Cocos2d-x 2.17 + Lua 原客户端：`client/ProjectX`。
- Unity + C# + Lua 迁移客户端：`unityclient`。
- 两套客户端共用同一套本地 MySQL + `kapai.exe` 游戏服。

运行时必须按当前任务最小化启停：

| 当前任务 | 服务端/MySQL | Cocos 客户端 | Unity/Unity MCP |
|---|---|---|---|
| 文档、代码阅读、静态修改 | 关闭 | 关闭 | 关闭 |
| 服务端编译、协议 Smoke | 需要时启动，验收后关闭 | 关闭 | 关闭 |
| Cocos 客户端联调 | 启动 | 启动 | 关闭 |
| Unity 场景/Prefab/PlayMode | 需要联调时启动 | 关闭 | 启动 |
| Unity 纯 C#/Lua 文件修改 | 关闭 | 关闭 | 默认关闭，编译验收阶段再启动 |

禁止同时常驻两套客户端。Unity、Unity Hub、Cocos Simulator、`kapai.exe` 和 workspace-local MySQL 不得因为上个任务使用过就继续常驻；阶段完成后关闭，下个任务按需重新启动。

默认不需要：

- Godot `funplay`
- Unity MCP（仅 Unity 场景、Prefab、Console、PlayMode 等编辑器操作阶段启用）
