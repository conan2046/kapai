...
param(
    ...
    [string]$EvidencePath = ".local/ui-fidelity/JingJie/fixture/jingjie-sqlite-fixture-snapshot.json",
    [string]$DatabasePath = ""
)

# Contract id: reversible-jing-jie-fixed-account
...
if (-not $DatabasePath.EndsWith("LocalServer\projectx.db", [StringComparison]::OrdinalIgnoreCase)) {
    throw "JingJie fixture only accepts Application.persistentDataPath/LocalServer/projectx.db."
}
if ($UserId -ne 7200057 -or $RoleId -ne 1000003) {
    throw "JingJie SQLite fixture identity must remain 7200057/1000003."
}
$running = @(Get-Process kapai, ProjectX, Unity -ErrorAction SilentlyContinue)
if ($running.Count -gt 0) { throw "Stop kapai.exe, ProjectX.exe and Unity.exe before JingJie SQLite fixture $Action." }

$evidence = if ([IO.Path]::IsPathRooted($EvidencePath)) { $EvidencePath } else { Join-Path $root $EvidencePath }
$backup = Join-Path $root ".local\unity-validation\jingjie-sqlite-fixture-current.snapshot"
& python -X utf8 (Join-Path $PSScriptRoot "Invoke-JingJieSqliteFixture.py") `
    --action $Action --database $DatabasePath --backup $backup --evidence $evidence `
    --user-id $UserId --role-id $RoleId
if ($LASTEXITCODE -ne 0) { throw "JingJie SQLite fixture adapter failed: $Action" }
