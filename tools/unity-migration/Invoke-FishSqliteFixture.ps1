...
param(
    ...
    [string]$EvidencePath = ".local/unity-validation/fish-sqlite-fixture-latest.json",
    [string]$DatabasePath = ""
)
...
if (-not $DatabasePath.EndsWith("AppData\LocalLow\Xuancai\ProjectX\LocalServer\projectx.db", [StringComparison]::OrdinalIgnoreCase)) {
    throw "Fish fixture only accepts Application.persistentDataPath/LocalServer/projectx.db."
}
if ($UserId -ne 7200057 -or $RoleId -ne 1000003) {
    throw "Fish SQLite fixture identity must remain 7200057/1000003."
}
$running = @(Get-Process kapai, ProjectX, Unity -ErrorAction SilentlyContinue)
if ($running.Count -gt 0) { throw "Stop kapai.exe, ProjectX.exe and Unity.exe before Fish SQLite fixture $Action." }

$evidence = if ([IO.Path]::IsPathRooted($EvidencePath)) { $EvidencePath } else { Join-Path $root $EvidencePath }
$backup = Join-Path $root ".local\unity-validation\fish-sqlite-fixture-backup.db"
& python -X utf8 (Join-Path $PSScriptRoot "Invoke-FishSqliteFixture.py") `
    --action $Action --database $DatabasePath --backup $backup --evidence $evidence `
    --settings (Join-Path $root "server\config\json\fish_settings.json") `
    --rewards (Join-Path $root "server\config\json\fish_reward.json") `
    --position (Join-Path $root "server\config\json\fish_position.json") `
    --user-id $UserId --role-id $RoleId
if ($LASTEXITCODE -ne 0) { throw "Fish SQLite fixture adapter failed: $Action" }
