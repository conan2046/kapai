...
param(
    ...
    [string]$EvidencePath = ".local/ui-fidelity/Bag/fixture/bag-sqlite-fixture-snapshot.json",
    [string]$DatabasePath = ""
)

# Contract id: reversible-bag-sqlite-fixed-account
...
if (-not $DatabasePath.EndsWith("LocalServer\projectx.db", [StringComparison]::OrdinalIgnoreCase)) {
    throw "Bag fixture only accepts Application.persistentDataPath/LocalServer/projectx.db."
}
if ($UserId -ne 1 -or $RoleId -ne 1000001) { throw "Bag SQLite fixture identity must remain 1/1000001." }
$running = @(Get-Process kapai, ProjectX, Unity -ErrorAction SilentlyContinue)
if ($running.Count -gt 0) { throw "Stop kapai.exe, ProjectX.exe and Unity.exe before Bag SQLite fixture $Action." }

$evidence = if ([IO.Path]::IsPathRooted($EvidencePath)) { $EvidencePath } else { Join-Path $root $EvidencePath }
$backup = Join-Path $root ".local\unity-validation\bag-sqlite-fixture-backup.db"
$python = Join-Path $PSScriptRoot "Invoke-BagSqliteFixture.py"
& python -X utf8 $python --action $Action --database $DatabasePath --backup $backup `
    --evidence $evidence --user-id $UserId --role-id $RoleId
if ($LASTEXITCODE -ne 0) { throw "Bag SQLite fixture adapter failed: $Action" }
