...
param(
    ...
    [string]$EvidencePath = ".local/unity-validation/answer-sqlite-fixture-latest.json",
    [string]$DatabasePath = ""
)
...
if (-not $DatabasePath.EndsWith("AppData\LocalLow\Xuancai\ProjectX\LocalServer\projectx.db", [StringComparison]::OrdinalIgnoreCase)) {
    throw "Answer fixture only accepts Application.persistentDataPath/LocalServer/projectx.db."
}
if ($UserId -ne 7200057 -or $RoleId -ne 1000003) {
    throw "Answer SQLite fixture identity must remain 7200057/1000003."
}
$running = @(Get-Process kapai, ProjectX, Unity -ErrorAction SilentlyContinue)
if ($running.Count -gt 0) { throw "Stop kapai.exe, ProjectX.exe and Unity.exe before Answer SQLite fixture $Action." }

$evidence = if ([IO.Path]::IsPathRooted($EvidencePath)) { $EvidencePath } else { Join-Path $root $EvidencePath }
$backup = Join-Path $root ".local\unity-validation\answer-sqlite-fixture-backup.db"
$settings = Join-Path $root "server\config\source\answer-settings.csv"
& python -X utf8 (Join-Path $PSScriptRoot "Invoke-AnswerSqliteFixture.py") `
    --action $Action --database $DatabasePath --backup $backup --evidence $evidence `
    --settings $settings --user-id $UserId --role-id $RoleId
if ($LASTEXITCODE -ne 0) { throw "Answer SQLite fixture adapter failed: $Action" }
