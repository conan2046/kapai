...
param(
    ...
    [string]$EvidencePath = ".local/ui-fidelity/Gameplay/fixture/gameplay-sqlite-fixture-snapshot.json",
    [string]$DatabasePath = ""
)
...
if (-not $DatabasePath.EndsWith("LocalServer\projectx.db", [StringComparison]::OrdinalIgnoreCase)) {
    throw "Gameplay fixture only accepts Application.persistentDataPath/LocalServer/projectx.db."
}
if ($UserId -ne 7200057 -or $RoleId -ne 1000003) {
    throw "Gameplay SQLite fixture identity must remain 7200057/1000003."
}
$running = @(Get-Process kapai, ProjectX, Unity -ErrorAction SilentlyContinue)
if ($running.Count -gt 0) {
    throw "Stop kapai.exe, ProjectX.exe and Unity.exe before Gameplay SQLite fixture $Action."
}
...
$python = Join-Path $PSScriptRoot "Invoke-GameplayFixedAccountFixture.py"
& python -X utf8 $python --action $Action --database $DatabasePath --backup $backup `
    --evidence $evidence --user-id $UserId --role-id $RoleId
if ($LASTEXITCODE -ne 0) { throw "Gameplay SQLite fixture adapter failed: $Action" }
