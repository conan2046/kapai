...
param(
    ...
    [ValidateSet("Setup", "SetupVisual", "AssertSetup", "AssertVisualSetup", "CaptureMutationHash", "AssertMutationReloginHash", "Restore", "AssertRestored", "Cleanup", "AssertCleanup", "AssertReloginHash")]
    [string]$Action,
    [uint32]$UserId = 1,
    [uint32]$RoleId = 1000001,
    [string]$EvidencePath = ".local/ui-fidelity/EnhanceMaster/fixture/enhance-master-fixed-fixture-snapshot.json",
    [string]$DatabasePath = ""
)
...
if (-not $DatabasePath.EndsWith("LocalServer\projectx.db", [StringComparison]::OrdinalIgnoreCase)) {
    throw "EnhanceMaster fixture only accepts the Unity LocalServer projectx.db path."
}

$evidence = if ([IO.Path]::IsPathRooted($EvidencePath)) { $EvidencePath } else { Join-Path $root $EvidencePath }
$backup = Join-Path $root ".local\unity-validation\enhance-master-sqlite-fixture-backup.db"
...
if ($running.Count -gt 0) { throw "Stop kapai.exe, ProjectX.exe and Unity.exe before EnhanceMaster SQLite fixture $Action." }
& python -X utf8 $python --action $Action --database $DatabasePath --backup $backup `
    --evidence $evidence --user-id $UserId --role-id $RoleId
if ($LASTEXITCODE -ne 0) { throw "EnhanceMaster SQLite fixture adapter failed: $Action" }
