...
$source = Join-Path $Root 'unitydata\server\source'
$excel = Join-Path $source 'excel\json'
$vendor = Join-Path $Root 'tools\unity-data\vendor\xl转表.exe'
...
foreach ($name in $authoritativeNames) {
    ...
    $authoritativeOverrides += [pscustomobject]@{
        path = $name
        source = ('unitydata/server/source/authoritative/json/{0}' -f $name)
...
$report = [ordered]@{
    schemaVersion = 1
    sourceExcel = 'unitydata/server/source/excel/json'
    converter = 'tools/unity-data/vendor/xl转表.exe'
    generatedOutput = 'unitydata/server/source/generated/json_server'
