...
local ArtifactUI = LUIBase:New()
ArtifactUI.__index = ArtifactUI
--local this = LTcpSocket
function ArtifactUI:New()
	local o = LUIBase:New()
	setmetatable(o,ArtifactUI)	
    o:Init()
	return o
end


function ArtifactUI:Init()
   ...
   local function onNodeEvent(event)        
    ...
    self:InitData()
    self:AddTouchEvt()
    if #LRoleDataMgr.m_ServerShenQiList > 0 then
        self:ShowShenQiAllAttr()
        self:ShowShenQiInfo()
        self:ShowRoleModel()
    else
        LuaNetSendMsg:QueryShenQiInfoNew(1)
    end

end

function ArtifactUI:onExit()
    self:Destory()
    self.m_pUILayer = nil
    self.m_pCell = nil
...
function ArtifactUI:RegistMsgs()
    self.msgIds = 
    {
        LUIShenQiEvent.CurShenQiChanged,
...
function ArtifactUI:ProcessEvent(msg)
    if msg.msgId == LUIShenQiEvent.CurShenQiChanged then
        self:UpdateCurShenQi(msg.value)
    elseif msg.msgId == LUIShenQiEvent.ShenQiStateChanged then
        self.m_newId = msg.value
        --self:UpdateShenQiState(msg.value)
    elseif msg.msgId == LUIShenQiEvent.GotShenQiList then
        self:UpdateInfo()
        if self.m_newId ~= nil and self.m_newId > 0 then
            self:UpdateShenQiState(self.m_newId)

        end
        self.m_newId = 0
    elseif msg.msgId == LUIShenQiEvent.ShenQiDevelopInfoChanged then
        self:ShowShenQiAllAttr()
    end
end

--更新当前神器的穿脱状态
function ArtifactUI:UpdateInfo()    
    local offset = self.m_pLeftTableView:getContentOffset() 
    self.m_pLeftTableView:reloadData()
    local min = self.m_pLeftTableView:minContainerOffset().y
    local max = self.m_pLeftTableView:maxContainerOffset().y
    local cellHeight = self.m_pCell:getContentSize().height
    offset.y = math.max(math.min(offset.y--[[+cellHeight]], max), min)
    self.m_pLeftTableView:setContentOffset(offset)
    --self:RefreshCell()
    self:ShowShenQiAllAttr()
    self:ShowShenQiInfo()
    self:ShowRoleModel()
...
--更新当前神器的穿脱状态
function ArtifactUI:UpdateCurShenQi(oldShenqiId)
   ...
   if oldShenqiId > 0 then
       self:UpdateShenQiState(oldShenqiId)
    end
    local shenqiId = LRoleDataMgr.MyHeroInfo.ShenQiId;
    if shenqiId > 0 then
        self:UpdateShenQiState(LRoleDataMgr.MyHeroInfo.ShenQiId)
    end
end

--更新当前神器的获得状态
function ArtifactUI:UpdateShenQiState(shenqiId)
    local info = LRoleDataMgr.m_ServerShenQiList
    if info == nil or #info == 0 then
        return
    end
    local curIdx = LRoleDataMgr:GetShenQiIndxById(shenqiId)
    if curIdx < 1 or curIdx > #info then
        return
    end
    local data = info[curIdx]
    
    local cell = self.m_pLeftTableView:cellAtIndex(curIdx-1)
    if cell == nil then
        Utils:MoveToTableIdx(self.m_pLeftTableView, self.m_pCell, curIdx-1)
        cell = self.m_pLeftTableView:cellAtIndex(curIdx-1)
        if cell == nil then
            return
        end
    ...
    if CellChild == nli then
        return
    end
    local stateLabel = CellChild:getChildByName("State")
    local signImage = CellChild:getChildByName("DressState")
    self:ShowShenQiState(stateLabel, data.state)
    self:ShowButtonText(data.state)
    ...
    self.m_newId = shenqiId
end

function ArtifactUI:InitData()
	...
	local pHelpBtn = attrPanel:getChildByName("btn_Help")
    ...
    self.m_btnRedDotImg = self.m_button:getChildByName("Prompt")

    self.m_listView:setScrollBarEnabled(false)
    self.m_allAttrLabels = {}
end

function ArtifactUI:helpButtonCallback()
    local str = GUITips.RSI_Help_Str6
...
function ArtifactUI:AddTouchEvt()
    local function btnTouched(sender)
        ...
        if data.state == 0 then
           LuaNetSendMsg:SendShenQiReq(6,data.id)
        elseif data.state == 1 then
            LuaNetSendMsg:SendShenQiReq(2,data.id,2)
        elseif data.state == 2 then
           LuaNetSendMsg:SendShenQiReq(2,0,1)
        end
    end
    self.m_button:addClickEventListener(btnTouched)  --神器穿、脱
	self:MarkIntaractCObj(self.m_button)
end

function ArtifactUI:RefreshCell()
    local info = LRoleDataMgr.m_ServerShenQiList
    self.m_curGridNum = #info;
    self.m_pLeftTableView:reloadData()
end

function ArtifactUI:InitLeftTabView(leftView)
...
--点击选中处理
function ArtifactUI:LeftTableCellTouched(cell)
...
function ArtifactUI:LeftTableCellAtIndex(sender, idx)
...
function ArtifactUI:ShowLeftCellInfo(cellChild, idx)
    --print("cell idx"..idx)
    local index = idx+1;
    ...
    if info[index].state == 2 then
        ...
        signImage:setVisible(false)
        if info[index].state == 0 and info[index].itemId > 0 and redDotImg ~= nil then
            local value = LRoleDataMgr.Equip:CountItemNumById(info[index].itemId) 
            if value >= info[index].needNum then
...
function ArtifactUI:ShowShenQiState(label,state)
    if label == nil then
        return
    end
    if state == 1 then
...
--isPer:是否显示%
function ArtifactUI:ShowShenQiAttr(idx,attr,val,isPer)
    local str = ""
    if attr ~= nil then
        str = attr..": "
...
function ArtifactUI:GetAttrValue(attrType)
...
function ArtifactUI:AddAttrValue(allAttr)
...
function ArtifactUI:ShowShenQiInfo()
    local info = LRoleDataMgr.m_ServerShenQiList
    if info == nil or #info == 0 or self.m_selectIdx < 0 or self.m_selectIdx + 1> #info then
        return
    end
...
--    self:ShowShenQiAttr(count,GUITips.UI_Shenqi_Zhanli,data.zhandouli)
   ...
   --按钮文字  data.state--1佩戴，2卸下， 0激活
   self:ShowButtonText(data.state)

   --按钮红点
   self.m_btnRedDotImg:setVisible(false)
   if data.state == 0 and data.itemId > 0 and self.m_btnRedDotImg  ~= nil then
       local value = LRoleDataMgr.Equip:CountItemNumById(data.itemId) 
       if value >= data.needNum then
          self.m_btnRedDotImg:setVisible(true)
       end
   end
end

--按钮文字  data.state--1佩戴，2卸下， 0激活
function ArtifactUI:ShowButtonText(state)
    if state == 1 then 
...
function ArtifactUI:ShowShenQiAllAttr()
    local info = LRoleDataMgr.m_ServerShenQiList
    if info == nil or #info == 0 then
       return
    end
...
--模型显示
function ArtifactUI:ShowRoleModel()
    local shenqiId = 0;
