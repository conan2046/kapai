...
local NormalDrawRewardUI = LUIBase:New()
NormalDrawRewardUI.__index = NormalDrawRewardUI
--local this = LTcpSocket
function NormalDrawRewardUI:New()
	local o = LUIBase:New()
	setmetatable(o,NormalDrawRewardUI)	
    o:Init()
	return o
end
...
--注册事件
-- -----------------------------------
function NormalDrawRewardUI:RegistMsgs()
    self.msgIds = 
    {
    }
...
-- -----------------------------------
function NormalDrawRewardUI:ProcessEvent(msg)

end

function NormalDrawRewardUI:Init()
...
function NormalDrawRewardUI:initData( ... )
    ...
    for i=1, #drawConfig do
        ...
        -- dump(data, "NormalDrawRewardUI:initData ===>")
    ...
    for i=7, 1, -1 do
        ...
        for j=1, #petList do
            ...
            if data and i == data.quality then
...
function NormalDrawRewardUI:initControlUI( ... )
    -- body
...
function NormalDrawRewardUI:GetTableSize(idx)
...
function NormalDrawRewardUI:InitTableView()
...
function NormalDrawRewardUI:TableCellAtIndex(sender, idx)
...
function NormalDrawRewardUI:ShowTableCellInfo(cellChild, idx)
...
--加载一个推荐的Icon列表
function NormalDrawRewardUI:ShowItemList(parent,tag,petIds)
...
--加载单个神将
function NormalDrawRewardUI:ShowItem(item,id)
...
function NormalDrawRewardUI:SetPosPercentY(node,flag,height)
...
function NormalDrawRewardUI:SetPosDifY(node,flag,height)
    if node == nil then return end
    local x,y = node:getPosition()
    y = height-flag
    node:setPosition(cc.p(x,y))
...
function NormalDrawRewardUI:CloseUI( ... )
    -- body
    Utils:DeleteUI("HappyDraw.NormalDrawRewardUI")
end

function NormalDrawRewardUI:onExit()
    self.m_pUILayer = nil
    self:Destory()
end

return NormalDrawRewardUI