...
local PetEquipMainUI = LUIBase:New()
PetEquipMainUI.__index = PetEquipMainUI
--local this = LTcpSocket
function PetEquipMainUI:New(openTab)
	local o = LUIBase:New()
	setmetatable(o,PetEquipMainUI)	
    o:Init(openTab)
	return o
end

function PetEquipMainUI:Init(openTab)
    self.Script = "PetEquip.PetEquipMainUI"
    self:RegistMsgs()
    ...
    --title
    LGameMsg.m_baseMsgWithOne:Change(LUIFClassBgEvent.SetTitle, GUITips.UI_Title_PetEquip)
    ...
    local tabValues = 
    {
        {GUITips.UI_Title_PetEquip_TabName1,GUITips.UI_Title_PetEquip_TabName2},
        tabBtnClicked
    }
    ...
    Utils:SendMsg(LUIFClassBgEvent.HelpBtn,AppDef.FCBHelp.ZhangBei) 
    self:OnEnter()
end

-- function PetEquipMainUI:RegisterGuide()
--     if true then
--         return
--     end
...
function PetEquipMainUI:RegistMsgs()
    self.msgIds = 
    {
...
function PetEquipMainUI:ProcessEvent(msg)
...
function PetEquipMainUI:onExit()
...
function PetEquipMainUI:OnEnter()
    self:InitRedDotState(RedDotDef.ID.ZBBeiBao)
    self:InitRedDotState(RedDotDef.ID.ZBSuiPian)
end

function PetEquipMainUI:InitRedDotState(id)
    if not self.m_pUILayer:isVisible() then
        return
    end
    local isShow = Utils:GetRedDotState(id)
    --print("PetEquipMainUI:InitRedDotState",id,isShow)
    self:DealUpdateRedDotState({id=id, isShow=isShow})
end

function PetEquipMainUI:DealUpdateRedDotState(data)
    local ind = 0
    ...
    if ind > 0 then
        --print("PetEquipMainUI:DealUpdateRedDotState",ind,data,isShow)
        Utils:SendMsg(LUIFClassBgEvent.RedDotState, {ind, data.isShow})
    end
end

function PetEquipMainUI:InitTouchEvt()
...
function PetEquipMainUI:HideCurUI()
...
function PetEquipMainUI:ShowCurUI()
    if self.m_pSubLayer[self.m_curUIInd] == nil then
...
function PetEquipMainUI:TabClicked(ind)
...
function PetEquipMainUI:DelayLoadSubUI(tabInd)
    --,"View.Social.MasterUI",
    local uinames = {"View.PetEquip.PetEquipBagSubUI","View.PetEquip.PetEquipPiecesSubUI"}
    local ind = tabInd
