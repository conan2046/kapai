local DonateManiUI = LUIBase:New()
DonateManiUI.__index = DonateManiUI

function DonateManiUI:New(openTab)
    local o = LUIBase:New()
    setmetatable(o,DonateManiUI)
    o:Init(openTab)
    return o
end

function DonateManiUI:Init(openTab)
    ...
    LGameMsg.m_baseMsgWithOne:Change(LUIFClassBgEvent.SetTitle, GUITips.UI_Title_Donate)
...
function DonateManiUI:onExit()
    self:Destory()
    self.m_pUILayer = nil
end

function DonateManiUI:InitTouchEvt()
...
function DonateManiUI:TabClicked(ind)
...
function DonateManiUI:HideCurUI()
...
function DonateManiUI:ShowCurUI()
...
function DonateManiUI:DelayLoadSubUI(tabInd)
    local uinames = {"View.Activity.DonateUI",}
    local ind = tabInd
