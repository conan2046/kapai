local TaskLayer = LUIBase:New()
TaskLayer.__index = TaskLayer

local WelfareActivityDef = require("View.WelfareActivity.WelfareActivityDef")
-----------------------------------
function TaskLayer:New(openTab)
    local o = {}
    setmetatable(o, TaskLayer)
    o:Init(openTab)
    return o
end
-----------------------------------
function TaskLayer:Init(openTab)
    self.Script = "Activity.TaskLayer"
    --------------------------------------------------
...
function TaskLayer:InitData(openTab)
    self.m_tabData = {};
...
-----------------------------------
function TaskLayer:InitUIControl()
 ...
 --        Utils:DeleteUI("WelfareActivity.TaskLayer")
 --    end
 --    closeBtn:addClickEventListener(closeEvent)
    ...
    self.m_pTabNodeCell:findChildByName("Button/Prompt"):setVisible(false);
    self.m_pListView = self.m_pUILayer:findChildByName("Panel_1/Btn_ListView");
end

-----------------------------------
function TaskLayer:RegistMsgs()
    self.msgIds = 
    {
...
-----------------------------------
function TaskLayer:ProcessEvent(msg)
...
-----------------------------------
function TaskLayer:InitTouchEvt()
...
function TaskLayer:onTiliClicked()
    Utils:OpenUseUI(500,1)
end

function TaskLayer:onMoneyClicked()
    print("onMoneyClicked")
    Utils:OpenFunction(AppDef.EModuleID.EMID_SCCHANGYONG)
end

function TaskLayer:onYuanbaoClicked()
    print("onYuanbaoClicked")
end

function TaskLayer:ShowTili()
...
function TaskLayer:ShowTongbao()
...
-----------------------------------
function TaskLayer:InitViewSize()
    self:CreateUINode("csd/huodong/huodong_bg.csb")
    self.m_pUILayer:findChildByName("Panel_1/Title/TitleName"):setString(GUITips.Title_Task_Name)
...
function TaskLayer:InitTabBtn()
    ...
    for i = 1, #tabArr do
        ...
        btn.userData = i;
        btnNode:findChildByName("Button/BtnName"):setString(data);
        btnNode:findChildByName("Button/ChooseBg/BtnName"):setString(data);
        btn:addClickEventListener(tabBtnClicked);
        table.insert(self.m_pTabBtns,btn);
    end
end

function TaskLayer:TabBtnClicked(ind)
...
function TaskLayer:SetBtnState()
...
function TaskLayer:ShowCurTab()
    self:SetBtnState();

    --local funcId = self._funcConfigs[self.m_openTab].function_id;

    --local funcMap = {[AppDef.EModuleID.EMID_TASK_DALIY] = "View.Activity.HuoyueTaskUI"}
    local funcMap = {"View.Activity.HuoyueTaskUI"}
    if self.m_pDelegates[self.m_openTab] == nil then
        local NewUI = require(funcMap[self.m_openTab])
...
-- function TaskLayer:TabClicked(ind)
...
-- function TaskLayer:TableCellAtIndex(sender, idx)
...
-- function TaskLayer:tableCellTouched(idx, updateImmediately)
--     if ind == self.m_selectIndex then
--         return
--     end
--     self:TabClicked(idx)
...
-- function TaskLayer:updateItem(cell, info, idx)
...
-- function TaskLayer:ReloadData(value)
--     local tag = value[1]
...
-- function TaskLayer:ReloadListData(list)
