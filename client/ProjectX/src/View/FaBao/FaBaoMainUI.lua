...
local FaBaoMainUI = LUIBase:New()
FaBaoMainUI.__index = FaBaoMainUI
--local this = LTcpSocket
function FaBaoMainUI:New(openTab)
	local o = LUIBase:New()
	setmetatable(o,FaBaoMainUI)	
    o:Init(openTab)
	return o
end

function FaBaoMainUI:Init(openTab)
    self.Script = "FaBao.FaBaoMainUI"
    self:RegistMsgs()
    ...
    --title
    LGameMsg.m_baseMsgWithOne:Change(LUIFClassBgEvent.SetTitle, GUITips.UI_Title_PetFaBao)
    self:SendMsg(LGameMsg.m_baseMsgWithOne)
    Utils:SendMsg(LUIFClassBgEvent.HelpBtn,AppDef.FCBHelp.FaBao)
    local function tabBtnClicked(ind)
        self:TabClicked(ind)
    end
    local tabValues = 
    {
        {GUITips.UI_Title_PetFaBao_TabName1,GUITips.UI_Title_PetFaBao_TabName2},
        tabBtnClicked
    }
...
-- function FaBaoMainUI:RegisterGuide()
--     if true then
--         return
--     end
...
function FaBaoMainUI:RegistMsgs()
    self.msgIds = 
    {
...
function FaBaoMainUI:ProcessEvent(msg)
...
function FaBaoMainUI:OnEnter()
    self:InitRedDotState(RedDotDef.ID.ZBBeiBao)
    self:InitRedDotState(RedDotDef.ID.ZBSuiPian)
end

function FaBaoMainUI:InitRedDotState(id)
...
function FaBaoMainUI:DealUpdateRedDotState(data)
    local ind = 0
...
function FaBaoMainUI:InitTouchEvt()
    ...
    local function closeCallback()
        print("closeCallback ====>", self.Script)
...
function FaBaoMainUI:HideCurUI()
...
function FaBaoMainUI:ShowCurUI()
    if self.m_pSubLayer[self.m_curUIInd] == nil then
...
function FaBaoMainUI:TabClicked(ind)
...
function FaBaoMainUI:DelayLoadSubUI(tabInd)
    --,"View.Social.MasterUI",
    local uinames = {"View.FaBao.FaBaoSubBagUI", "View.FaBao.FaBaoSuiPianBagUI"}
    local ind = tabInd
...
function FaBaoMainUI:UpdateRedDotUI()
	local isfabao = Utils:GetRedDotState(RedDotDef.ID.FBBeiBao)
	Utils:SendMsg(LUIFClassBgEvent.RedDotState, {1, isfabao})
	local issuipian = Utils:GetRedDotState(RedDotDef.ID.FBSuiPian)
	Utils:SendMsg(LUIFClassBgEvent.RedDotState, {2, issuipian})
end
