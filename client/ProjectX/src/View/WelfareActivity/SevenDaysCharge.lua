...
local SevenDaysCharge = LUIBase:New()
SevenDaysCharge.__index = SevenDaysCharge
local TimerLabelUI = require("View.Common.TimerLabelUI")
--local this = LTcpSocket
function SevenDaysCharge:New(parent)
	local o = LUIBase:New()
	setmetatable(o,SevenDaysCharge)	
    o:Init(parent)
	return o
end
...
--注册事件
-- -----------------------------------
function SevenDaysCharge:RegistMsgs()
    self.msgIds = 
    {
...
-- -----------------------------------
function SevenDaysCharge:ProcessEvent(msg)
    -- if msg.msgId == LUIWelfareEvent.updateSevenCharge then
    --     print("SevenDaysCharge 7777777777777777")
    --     self:updateData()
    -- end
...
function SevenDaysCharge:Init(parent)
...
function SevenDaysCharge:onExit()
    self.m_pUILayer = nil
    self:Destory()
    local _ = self.m_timerLabel and self.m_timerLabel:Destory()
end

function SevenDaysCharge:InitUIControl()
...
function SevenDaysCharge:initData( ... )
    -- body
...
function SevenDaysCharge:updateData( ... )
    -- body
    self.m_pUILayer:setVisible(true)
    local totleData = LRechargeDataMgr:GetSevenRechargeData()
    local dataInfo = totleData.m_dayItemDataArr
    print("SevenDaysCharge:updateData ==>", totleData.m_strDesc, m_leftTime)
    -- self._time:setString(totleData.m_leftTime)
    local str = string.format("%d/%d", totleData.m_curValue, totleData.m_curLimit)
    self._curChargeStr:setString(str)
    self._curChargeStr2:setString(tostring(totleData.m_curLimit))
    local function getAwardEvent(sender )
        -- body
...
--    local index = LRechargeDataMgr:getFirstAwardIndex()
    ...
    print("updateOtherAwardUI ==>", totleData.m_leftTime)
    self:setTimer(totleData.m_leftTime)
...
function SevenDaysCharge:setLabel(label)
    if label ~= nil then
        self.m_timerLabel = TimerLabelUI:New(label, nil, nil, handler(self, self.TimeReduce))
    end
end

function SevenDaysCharge:TimeReduce(pText, h, m, s, left)
    if pText == nil then
        return
    end
...
function SevenDaysCharge:setTimer(leftTime)
...
function SevenDaysCharge:updateDayAwardBtn( ... )
    -- body
...
function SevenDaysCharge:refrashOtherAwardUI( ... )
    -- body
...
function SevenDaysCharge:Reset( )
    -- body
    self.m_pUILayer:setVisible(false)
end


return SevenDaysCharge