...
MonsterName = LANGUAGE_TRANSFORM_2003
LEVEL_LIMIT = 30
...
function NpcMain(pUser,missionId,index)
	...
	Option(pUser,MonsterName,LANGUAGE_TRANSFORM_2004,LANGUAGE_TRANSFORM_2005)
...
function MonsterCallBack(pUser,sel)
	if sel == 1 then
		if pUser:GetLevel() < LEVEL_LIMIT then
			Dialog(pUser,MonsterName,LANGUAGE_TRANSFORM_2006..LEVEL_LIMIT..LANGUAGE_TRANSFORM_2007)
			return
		end
