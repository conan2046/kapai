...
function NpcMain(pUser,missionId)
	...
	if s ~= nil then
		...
		if tonumber(t[1]) == 4 then
			if tonumber(t[6]) == 4 then
				j.ShowFinishYaYunBiaoChePanel(pUser)
--				Option(pUser,"恭镖头","三思，三思，再三思。","3|完成押运镖车")
			else
				j.ShowYaYunBiaoChePanel(pUser,0,165)			
--				Option(pUser,"恭镖头","三思，三思，再三思。","1|我要换车|2|不换车")
			end
...
--			j.ShowYaYunBiaoChePanel(pUser)
		...
		Dialog(pUser,NPCName,LANGUAGE_TRANSFORM_4702)
	end
end
...
--		end
		...
		if q == 4 or (q == 3 and math.random(1,100) <= 20) then
			j.SysInfoToAllUser("[c"..ROLE_NAME_COLOR.."]"..pUser:GetName()..LANGUAGE_TRANSFORM_4703..ITEM_NAME_COLOR.."]"..j.GetSceneName(pUser:GetSceneId())..LANGUAGE_TRANSFORM_4704..GongGaoColor[q][1].."]"..GongGaoColor[q][2]..LANGUAGE_TRANSFORM_4705)
		end
		...
		if q >= 1 then
			j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_4706..GongGaoColor[q][2]..LANGUAGE_TRANSFORM_4707)
		else
			j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_4708)
		end
		...
		if worldExpPer > 0 then
			...
			j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_4713..exp..LANGUAGE_TRANSFORM_4714..money..LANGUAGE_CHY_301..j.GetItemName(item_id).."*"..item_num.."[/c]")
		end
...
--		j.ShowYaYunBiaoChePanel(pUser,0)
	elseif sel == 3 then
		...
		j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_4716)
		pUser:RecoveryAllHp()
		pUser:AddExpByItemWithTips(exp)
--		pUser:AddBaiHuaChip(3,1,1)
		
		pUser:AddMoney(money)
		j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_4717..money.."[/c]")
