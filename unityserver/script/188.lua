...
function NpcMain(pUser,missionId,index)
	...
	if lv >= XCS_LEVEL_LIMIT then
		Option(pUser,name,LANGUAGE_TRANSFORM_4589..name..LANGUAGE_TRANSFORM_4591,LANGUAGE_TRANSFORM_4592)
	else
		Option(pUser,name,LANGUAGE_TRANSFORM_4593..name..LANGUAGE_TRANSFORM_4595,LANGUAGE_TRANSFORM_4596)
	end
...
function MonsterCallBack(pUser,sel)
	...
	if sel == 1 then
		if pUser:GetLevel() < LEVEL_LIMIT then
			Dialog(pUser,name,LANGUAGE_TRANSFORM_4597..LEVEL_LIMIT..LANGUAGE_TRANSFORM_4598)
			return
		end
		
		if pUser:GetTeam() == 0 then
			Dialog(pUser,name,LANGUAGE_TRANSFORM_4599)
			return
		end
		if pUser:GetTeam() ~= pUser:GetRoleId() then
			Dialog(pUser,name,LANGUAGE_TRANSFORM_4600)
			return
		end
		if j.GetTeamMemNum(pUser) < 2 then
			Dialog(pUser,name,LANGUAGE_TRANSFORM_4601)
			return
		end
		...
		if pUser:IsXunChaShiKilled(NPCID,index) then
			Dialog(pUser,name,LANGUAGE_TRANSFORM_4606)
			return
		end
