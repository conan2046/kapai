...
function NpcMain(pUser,missionId)
	...
	if s ~= nil then
		...
		if tonumber(t[1]) == 5 then
			if tonumber(t[6]) == 5 then
				j.ShowFinishYaYunBiaoChePanel(pUser)
--				Option(pUser,"陆镖头","有点意思！","3|完成押运镖车")
			else
				j.ShowYaYunBiaoChePanel(pUser,0,166)
--				Option(pUser,"陆镖头","有点意思！","1|我要换车|2|不换车")
			end
...
--			j.ShowYaYunBiaoChePanel(pUser)
		...
		Dialog(pUser,LANGUAGE_TRANSFORM_986,LANGUAGE_TRANSFORM_987)
	end
end
...
--		end
		...
		if q == 4 or (q == 3 and math.random(1,100) <= 20) then
			j.SysInfoToAllUser("[c"..ROLE_NAME_COLOR.."]"..pUser:GetName()..LANGUAGE_TRANSFORM_988..ITEM_NAME_COLOR.."]"..j.GetSceneName(pUser:GetSceneId())..LANGUAGE_TRANSFORM_989..GongGaoColor[q][1].."]"..GongGaoColor[q][2]..LANGUAGE_TRANSFORM_990)
		end
		...
		if q >= 1 then
			j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_991..GongGaoColor[q][2]..LANGUAGE_TRANSFORM_992)
		else
			j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_993)
		end
		...
		if worldExpPer > 0 then
			...
			j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_998..exp..LANGUAGE_TRANSFORM_999..money..LANGUAGE_CHY_301..j.GetItemName(item_id).."*"..item_num.."[/c]")
		end
...
--		j.ShowYaYunBiaoChePanel(pUser,0)
	elseif sel == 3 then
		...
		j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_1001)
		pUser:RecoveryAllHp()
		pUser:AddExpByItemWithTips(exp)
--		pUser:AddBaiHuaChip(3,1,1)
		
		pUser:AddMoney(money)
		j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_1002..money.."[/c]")
