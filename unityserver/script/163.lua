...
function NpcMain(pUser,missionId)
	...
	if s ~= nil then
		...
		if tonumber(t[1]) == 3 then
			if tonumber(t[6]) == 3 then
				j.ShowFinishYaYunBiaoChePanel(pUser)
--				Option(pUser,"吕镖头","替我问候你主治大夫！","3|完成押运镖车")
			else
				j.ShowYaYunBiaoChePanel(pUser,0,164)
--				Option(pUser,"吕镖头","替我问候你主治大夫！","1|我要换车|2|不换车")
			end
...
--			j.ShowYaYunBiaoChePanel(pUser)
		...
		Dialog(pUser,NPCName,LANGUAGE_TRANSFORM_2094)
	end
end
...
--		end
		...
		if q == 4 or (q == 3 and math.random(1,100) <= 20) then
			j.SysInfoToAllUser("[c"..ROLE_NAME_COLOR.."]"..pUser:GetName()..LANGUAGE_TRANSFORM_2095..ITEM_NAME_COLOR.."]"..j.GetSceneName(pUser:GetSceneId())..LANGUAGE_TRANSFORM_2096..GongGaoColor[q][1].."]"..GongGaoColor[q][2]..LANGUAGE_TRANSFORM_2097)
		end
		...
		if q >= 1 then
			j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_2098..GongGaoColor[q][2]..LANGUAGE_TRANSFORM_2099)
		else
			j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_2100)
		end
		...
		if worldExpPer > 0 then
			...
			j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_2105..exp..LANGUAGE_TRANSFORM_2106..money..LANGUAGE_CHY_301..j.GetItemName(item_id).."*"..item_num.."[/c]")
		end
...
--		j.ShowYaYunBiaoChePanel(pUser,0)
	elseif sel == 3 then
		...
		j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_2108)
		pUser:RecoveryAllHp()
		pUser:AddExpByItemWithTips(exp)
--		pUser:AddBaiHuaChip(3,1,1)
		
		pUser:AddMoney(money)
		j.SendSysInfo(pUser,LANGUAGE_TRANSFORM_2109..money.."[/c]")
